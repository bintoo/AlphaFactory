...
class MyAgentStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        # Model state (OLS/Ridge on engineered features)
        self.theta = None
        ...
        # Strategy parameters (validation is rolling; refit yearly)
    ...
    def RecoverState(self):
        # Rebuild any in-memory state from Portfolio so the algorithm can resume safely.
        self.latestPred = {}
        self.targetWeights = {}
        ...
        for s in self.symbols:
            ...
            # If invested, keep a neutral "hold current quantity" target weight initially.
            # (We will overwrite targets on the next monthly rebalance.)
            p = self.Portfolio[s]
            if p.Invested:
                # approximate current weight using last known price if available
    ...
    def OnData(self, data: Slice):
        ...
        # Build monthly panel with STRICT no-lookahead alignment:
        # Use features known as of end of month t-1, and predict return of month t.
        ...
        # xs returns a DataFrame indexed by symbol for that month
    ...
    def ExitLogic(self, data: Slice):
        # No explicit exit logic specified by the concept; placeholder remains.
        pass
    ...
    def _BuildMonthlyPanel_NoLookahead(self):
        ...
        # Build per-symbol monthly close and monthly return (mret) from history
        ...
        # Compute macro x_t for month t using monthly returns of month t-1
        ...
        # Rank-map characteristics cross-sectionally into [-1, 1] per month; fill missing with cross-sectional median.
        for c in char_cols:
        ...
        # Feature engineering: z = x ⊗ c (8*6=48) plus base macro + base chars
    ...
    def _ComputeMonthlyMacro_FromMonthlyReturns(self, monthly_ret_by_sym: dict):
        # Macro for month t is constructed from monthly returns of month t-1 (known at start of month t).
        required = ["SPY", "QQQ", "IWM", "EFA", "EEM", "TLT", "IEF", "GLD", "VNQ", "XLE"]

        # Map ticker -> Symbol without using string(Symbol) heuristics
        ticker_to_symbol = {}
        for s in self.symbols:
    ...
    def _SetDecileTargetsAndPlaceLimitOrders(self, data: Slice):
        ...
        # Place limit orders at last price snapshot (execution left abstract)
        for s in self.symbols:
            if not data.Bars.ContainsKey(s):
                continue
            ...
            tgt_w = self.targetWeights.get(s, 0.0)
